...
const InteractiveBodyWeightChart: React.FC<InteractiveBodyWeightChartProps> = ({
	selectedRange,
}) => {
	...
	useEffect(() => {
		const generateDataForRange = () => {
			setIsLoading(true);
			const numDays =
				timeRangeOptions.find((opt) => opt.value === selectedRange)?.days || 7;
			...
			if (step > 0) {
				...
				setYAxisLabels([
					calculatedMaxValue.toFixed(1),
					calculatedMinValue.toFixed(1),
				]);
			}
	...
	if (chartData.length === 0) {
		return (
			<Text style={styles.loadingText}>No data available for this range.</Text>
		);
	}
	...
	return (
		<View style={styles.chartContainer}>
			<LineChart
				...
				spacing={
					chartData.length > 1
						? (chartWidth - 20 - endPadding) / (chartData.length - 1)
						: chartWidth - 20 - endPadding
				} // Adjusted spacing for wider chart
				textColor="#8E8E93"
				textFontSize={10}
				color="#0057ff"
				...
				yAxisTextStyle={{ color: 'white', fontSize: 10, marginRight: 5 }}
